...
def _norm_na(value):
    """Homologa los valores 'vacíos' o 'no aplica' de la Ficha Técnica a 'NO_APLICA'.

    Estandariza expresiones equivalentes (N/A, NA, N.A., '', None...)
    a un único término 'NO_APLICA' para consistencia visual en toda la ficha.
    """
    s = str(value or "").strip()
    if s.upper().replace(".", "").replace(" ", "").replace("_", "") in ("", "NA", "NOAPLICA", "N/A", "N/A."):
        return "NO_APLICA"
    return s
...
def generate_implementation_pdf(quote: dict, client: dict, contacts: list, branches: list, logo_path: str = None) -> bytes:
    ...
    client_name = client.get('legal_name') or client.get('fantasy_name') or 'NO_APLICA'
    ...
    quote_type = quote.get('quote_type', 'NO_APLICA')
    ...
    elements.append(_key_value_table([
        ...
        ("RIF", format_rif(client.get('rif', 'NO_APLICA')) or 'NO_APLICA'),
    ...
    tech_pairs = [
        ("Nombre del Integrador", quote.get('integrator_name', 'NO_APLICA')),
        ("Nombre del Aplicativo", quote.get('integrator_app_name', 'NO_APLICA')),
        ("Modelo de Pinpad", quote.get('pinpad_model', 'NO_APLICA')),
        ("Patrocinador de Pinpads", pinpad_sponsor or 'NO_APLICA'),
    ]
    ...
    tech_pairs.append(("Patrocinador de la Implementacion", impl_sponsor or "NO_APLICA"))
    ...
    if all_serials:
        ...
        for eq in all_serials:
            ...
            eq_data.append([
                Paragraph(str(eq.get("modelo", "NO_APLICA")), styles['SmallText']),
                Paragraph(str(eq.get("serial", "NO_APLICA")), styles['SmallText']),
    ...
    if is_payment_gateway and pg_setup_items:
        ...
        for idx, item in enumerate(pg_setup_items, 1):
            concepto = str(item.get('concepto') or item.get('item_name') or 'NO_APLICA')
            banco = str(item.get('banco') or 'NO_APLICA')
        ...
        if additional_items:
            ...
            for item in additional_items:
                bank_data.append([
                    Paragraph(str(item.get('bank_name', 'NO_APLICA')), styles['SmallText']),
                    Paragraph(str(item.get('item_name', item.get('name', 'NO_APLICA'))), styles['SmallText']),
    ...
    if contacts:
        for c in contacts:
            name = c.get('full_name') or f"{c.get('first_name', '')} {c.get('last_name', '')}".strip() or 'NO_APLICA'
            contact_data.append([
                Paragraph(str(name), styles['SmallText']),
                Paragraph(str(c.get('role', c.get('position', 'NO_APLICA'))), styles['SmallText']),
                Paragraph(str(c.get('phone', 'NO_APLICA')), styles['SmallText']),
                Paragraph(str(c.get('email', 'NO_APLICA')), styles['SmallText']),
            ])
    else:
        contact_data.append([
