...
**Mejora · Modal Asignación de Servidor (Enviar a Implementación): Grupo Económico como DropList + prellenado desde ficha · 2026-06:**
- Requerimiento: en la fase consolidada, el Nombre de Fantasía y el Grupo Económico deben prellenarse desde la Ficha del Cliente si están cargados; si vacíos, Fantasía es texto libre y Grupo Económico un DropList de grupos registrados (blanco si el cliente no tiene grupo).
- El prellenado desde la ficha ya existía (Quotes.jsx setEconomicGroup/setFantasyName desde c.grupo_economico / c.fantasy_name en los handlers de envío a implementación).
- Cambio (QuoteModals.jsx, fase 'consolidated_data'): el campo Grupo Económico pasó de Input libre a un <select> (DropList) alimentado del endpoint /grupos-economicos (catalogEconomicGroups, cargado al abrir el modal junto a servers/fiscal-printers). Incluye opción '— Sin Grupo Económico —' y conserva como opción el valor heredado de la ficha aunque no esté en el catálogo. Nombre de Fantasía se mantiene como Input editable (prellenado).
- data-testid: consolidated-economic-group-select (antes -input).
- Verificado: endpoint /grupos-economicos (46 grupos) + smoke de compilación de /quotes (sin crash). NOTA: la interacción dentro del modal consolidado no se probó E2E por depender del flujo completo de Enviar a Implementación; los componentes/datos sí fueron verificados.

**Fix + Mejora · Fecha congelada en Ficha Técnica + Modal Destinatarios agrupado por origen · 2026-06:**
- FIX Fecha Ficha Técnica: antes usaba datetime.now() (fecha del día). Ahora se CONGELA a la fecha de creación del proyecto. services/implementation_pdf.py usa quote['ficha_emission_date'] (parser _parse_emission_date) para encabezado 'Fecha:' y pie; fallback a hoy solo si no viene. projects.py (endpoint ficha-tecnica) y direct_projects.py pasan ficha_emission_date = project.created_at.
- Mejora Modal 'Seleccionar Destinatarios' (QuoteModals.jsx): los contactos se muestran en SECCIONES con encabezado/límite visible según scope: 'Grupo Económico' (grupo), 'RIF Principal' (principal), 'Sucursal' (local). Cada sección con título + conteo; se oculta si vacía. Se conserva selección por checkbox y contador. data-testids: contact-select-groups, contact-group-grupo/principal/local.
- Verificado: testing_agent iter334 (100% backend + 100% frontend): PDF de prj_826643491b64 muestra '29 de abril de 2026' (creación, no hoy) + otros proyectos pasados OK; modal agrupado con encabezado 'RIF PRINCIPAL (1)' y checkbox/contador funcionando.

**Estándar · "NO APLICA" en PDFs + Respaldo de nuevas colecciones · 2026-06:**
- Reversión/estándar de texto: los campos vacíos que antes mostraban "N/A" ahora muestran "NO APLICA" en TODOS los PDFs. Cambios: services/implementation_pdf.py (_norm_na retorna 'NO APLICA'; detector sigue reconociendo entradas 'N/A'/'NA'/'NOAPLICA'; todos los fallbacks 'N/A'→'NO APLICA'), billing_pdf.py, transfer_note_pdf.py, hoja_ruta_pdf.py, temporary_assignment_pdf.py.
- RIF (por decisión del usuario): rif_formatter.py (backend) y utils/rifFormatter.js (frontend) ahora homologan placeholders vacíos ('N/A','NA','-','SIN RIF', vacío) a 'NO APLICA' al mostrar. La matriz de certificación de Integradores (estados cortos P/C/N/A) NO se tocó por decisión del usuario.
- Respaldos por segmento: se agregaron 2 nuevas colecciones al Centro de Respaldos (data_migration.py MODULES + BACKUP_CENTER_ENTITIES): 'servers' → colección servers / key server_id (label 'Servidores'); 'fiscal-printers' → colección fiscal_printer_models / key model_id (label 'Impresoras Fiscales'). El frontend BackupCenter.jsx los lista dinámicamente vía /admin/backup-center/entities.
- Verificado (curl): entities lista Servidores(3) e Impresoras Fiscales(3); export por módulo OK; export-zip genera servers.json + fiscal-printers.json + _manifest.json. rif_formatter('N/A')→'NO APLICA'.

**Feature · Depuración MPOS + Filtrado de Integradores por integration_type + Badges [PG]/[LP]/[TK] · 2026-06:**
- (A) 'MPOS (Tablet/Móvil)' (id MPOS) ocultado del menú de creación de Cotizaciones (constants.js: disabled:true; el wizard filtra .filter(t=>!t.disabled)). Se conserva 'MPOS (Imple + POS)' (FAST_TRACK). Aplica a PyME y Corporativo (mismo QuoteWizardDialog).
- (B) Filtrado dinámico del selector de Integradores por Tipo de Proyecto usando integration_type (antes era integration_modality): VPOS/VPOS_MULTIRIF→CR, MPOS/FAST_TRACK→MP, GATEWAY→PG, LINK_PAGO/LINK→LP+TK (combinado). Centralizado en utils/integratorModality.js.
...
**AJUSTE · profiles+users en grupo modular EXCLUSIVO · 2026-06:**
