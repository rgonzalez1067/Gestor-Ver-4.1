...
- **Nuevo campo `last_followup_at` ("Último Seguimiento"):** mide la gestión global del proyecto. Se actualiza (+`last_followup_by`) en: notificaciones cliente/banco (`_send_sequential_notification`), adhoc, y **cualquier comentario de bitácora** (`add_bitacora_entry`, vía `$push`+`$set`). Se diferencia de `last_contact_at` ("Último Contacto"), que solo lo mueven las comunicaciones al Cliente (cliente + adhoc), NO la bitácora.
- **Frontend:** en la columna "Implementador" de la grilla, la etiqueta bajo el nombre pasó de "Último contacto" (last_contact_at) a **"Último Seguimiento"** (`last_followup_at`) — `ImplementerWorkloadHover.jsx` (prop `lastFollowupAt`, testid `last-followup-{id}`). La columna "Envío a Imple / Último Contacto" se mantiene con su Último Contacto.
- **QA (curl + screenshot):** Prueba A (bitácora) → `last_followup_at`=hoy, `last_contact_at` y `last_qualified_activity_at` SIN cambio ✓. Prueba B (notificación cliente) → los 3 campos=hoy y `business_days_in_state`=0 → **barra verde** ✓. Screenshot: proyecto Kraken con "Último Seguimiento: 21/8/2026" y barra verde ("En gestión · 0d háb.") vs resto en rojo. ⚠️ PREVIEW; requiere REDEPLOY.
...
- **Palette del editor:** además se agregó `Detalles_Recepcion_Equipos` (+ `Cantidad_Equipos`, `Equipos_Recibidos_HTML`, `Fecha_Recepcion`) a la categoría **"Despacho / Reparación / Equipos"** en `src/components/email/templateVariables.js` (fuente única consumida por EmailTemplatesEditor y TemplatesAdminDialog), para que sea un **chip insertable con un clic** en el Panel de Variables.
- **QA:** self-test backend — render `_render('{Detalles_Recepcion_Equipos}')` substituye OK; variable presente en `/api/other-actions/metadata` y `/api/other-actions/catalog`; POST `/api/taller/recepcion` con `detalles_recepcion` → 200 (con cleanup). Screenshots: textarea en la posición correcta (contador 71/500) y el chip `{Detalles_Recepcion_Equipos}` presente en el Panel de Variables del editor. ⚠️ PREVIEW; requiere REDEPLOY. Nota: para que Operaciones vea el texto, el admin debe insertar `{Detalles_Recepcion_Equipos}` en la plantilla del evento en Configuración → Otras Acciones.
...
- **QA:** testing_agent iter306 → **backend 100% (3/3 pytest) + frontend 100% (2/2)**. Verificado: (A) contacts[0] → correo garantizado a integrador.desarrollo@cliente.com con Certificado+anexo y variables resueltas; (B) regresión principal_contact_email OK; (C) sin email → 400 y no cierra; + regresión FALLA A (adjuntos en Centro de Mensajes) OK. Test: `/app/backend/tests/test_iter306_integrator_close_contacts_mapping.py`. ⚠️ PREVIEW; requiere REDEPLOY.


### Bug Fix: Anexos en Centro de Mensajes (Cierre/MPOS) + Correo al Responsable del Integrador — Jul 2026
- **Falla A (anexos perdidos en el Centro de Mensajes):** la rama `channel=='inbox'` de `services/other_actions_engine.py::dispatch_other_action` llamaba a `deliver_to_inbox` SIN pasar `attachments` → las notificaciones del Cierre de Proyecto de Integración llegaban al inbox sin adjuntos (attachments_meta sin content_b64 → descarga 410). **Fix:** ahora pasa `attachments=extra_attachments` (y `project_id`). La ruta MPOS "Marcar como entregada" usa `notification_engine.try_dispatch` que YA pasaba attachments al inbox (L616), por lo que comparte el conector ya validado.
- **Falla B (no llegaba correo al Responsable del Integrador):** en `routes/integrators.py::close_integrator_project`, la lista `guaranteed_close` incluía `contacts[].email` + `integrator.email` pero NO `principal_contact_email` (el correo de "Responsable por parte del Integrador"). **Fix:** se agrega `principal_contact_email` a `guaranteed_close` (recibe SIEMPRE, independiente de la config de Otras Acciones) y se mapean las variables de plantilla `usuario_integrador`/`Usuario_Integrador` → `principal_contact_email`.
- **QA:** testing_agent iter305 → **backend 100% (2/2 pytest)**. Verificado E2E contra Mongo real + endpoint público: (1) el inbox del destinatario del cierre trae 2 adjuntos (Certificado + anexo) con content_b64; (2) `GET /api/inbox/{id}/attachments/{idx}` devuelve 200 con el binario correcto (no 410); (3) email_logs con envío garantizado a integrador_prueba@comercio.com con adjuntos; (4) variables usuario_integrador resueltas. Test: `/app/backend/tests/test_iter305_inbox_attachments_and_integrator_email.py`. MPOS deliver no se probó E2E (complejidad de seed) pero comparte el conector `deliver_to_inbox(attachments=...)` verificado. ⚠️ PREVIEW; requiere REDEPLOY para producción.
...
- **Cómo rescatar en PRODUCCIÓN (Deploy):** tras redeployar, un admin ejecuta el rescate con un clic desde **Configuración → Configuración General → "Rescatar Correos Antiguos"** (botón `run-bitacora-backfill-btn`, solo admin), que llama al endpoint y muestra cuántos correos/proyectos se convirtieron. Como el HTML sigue almacenado, la conversión es posible sin pérdida.
...
- **Frontend** (`OtherActionsConfig.jsx`): opción "📧 Correo del Cliente" como primer item del selector de tipo; etiqueta "— correo del cliente externo —"; canal forzado a Email (cliente externo, sin Centro de Mensajes).
- **QA:** self-test — resolver unit (4 casos), dispatch E2E con `send_email` mockeado (cliente = destinatario TO directo, CC de-dup OK, usuario interno también recibe), screenshot del dropdown mostrando la opción. Backend syntax OK, salud 200. ⚠️ PREVIEW; requiere REDEPLOY.
- **Bug fix (Jul 2026):** al Guardar una fila `client_field` el backend devolvía "Tipo de destinatario inválido: client_field". **RCA:** el endpoint `PUT /api/other-actions/configs` (`other_actions_config.py` L277) tenía una whitelist `ALLOWED_RECIPIENT_TYPES` sin `client_field`. **Fix:** se agregó `client_field` a la whitelist y se fuerza `delivery_channel='email'` si el operador elige inbox (cliente externo). **QA:** testing_agent iter290 → backend 100% (7/7), sin residuales. Test: `/app/backend/tests/test_iter290_client_field_recipient.py`.


### Bug Fix: Seriales perdidos en correo al cliente + Autoría "Sistema" en anexos (Cotizaciones de Reparación) — Jul 2026
- **Falla A (seriales perdidos en el correo):** el PDF mostraba los seriales pero el correo al cliente llegaba sin ellos. **RCA:** `notification_engine.py` L217 leía la clave `repaired_models` (inexistente) en vez de `repair_models` (la persistida en Mongo) → `Lista_Seriales`/`lista_modelos_seriales`/`modelos_resumen` quedaban vacíos; además la plantilla `repair_quote_sent_PYME` solo citaba `{modelos_resumen}` (no `{Lista_Seriales}`). **Fix:** (1) leer `repair_models` (con fallback a `repaired_models`); (2) auto-inyección en el motor: para `action_id=='send_to_client'` + `quote_category=='repair'`, si el cuerpo renderizado NO contiene ya los seriales, se inserta un bloque "Detalle de Seriales de los Equipos" antes del footer (paridad total con el PDF).
- **Falla B (autoría "Sistema" en anexos):** el metadato `uploaded_by_name` de los anexos registraba "Sistema" en vez del usuario autenticado. **RCA:** `full_name` es `None` en los 45 usuarios; el código usaba `current_user.get("full_name","Sistema")` (o `first+last` sin fallback) → "Sistema"/vacío. **Fix:** cascada robusta `full_name → name → first+last → email → "Sistema"` aplicada a los 4 puntos de creación de anexos en `quotes.py` (L391, L1304, L2360, L2604) y al `status_history[0].user` (L2418).
- **QA:** testing_agent iter288 → **backend 100% (4/4 pytest)**. Verificado: `Lista_Seriales` renderiza los 3 seriales; el motor auto-inyecta el bloque con la plantilla real; `uploaded_by_name` y `status_history.user` = 'Rafael González' (nunca 'Sistema'). Test: `/app/backend/tests/test_iter288_repair_seriales_and_autoria.py`.
- **⚠️ Nota operativa (config, no código):** la config dinámica `reparaciones|_|send_to_client` en este entorno solo tiene 3 destinatarios `type='user'` (internos) y NINGÚN `type='client_field'`. Si el cliente debe recibir el correo, el admin debe agregar una fila `client_field` en Configuración → Otras Acciones; el código ya inyecta los seriales cuando el cliente recibe el correo. ⚠️ PREVIEW; requiere REDEPLOY.
...
- **Mejora (corrección manual de estatus — SOLO admin):** nuevo `PUT /api/taller-equipos/{id}/estatus` (403 si no es admin) que permite cambio LIBRE entre los 4 estatus válidos (`Recibido`/`Cotizado`/`En reparación`/`Entregado`); al pasar a `Entregado` registra `fecha_entrega`, al salir la limpia; registra bitácora `taller_estatus_corregido` (anterior/nuevo/motivo/usuario). Motivo opcional, ajuste silencioso (sin correos). Frontend `TallerEquipos.jsx`: botón lápiz por fila (admin) → modal `status-edit-modal` (`status-edit-select`, `status-edit-motivo`, `status-edit-save`); badge de estatus con 4 colores (Recibido=ámbar, Cotizado=índigo, En reparación=cian, Entregado=verde). Verificado por curl (Entregado set/clear fecha, 400 estatus inválido, bitácora) + screenshot del modal.
...
- Backend `services.py` L193: `valid_tipo_corp` incluye "Infraestructura" (validación de importación).
- Backend `pdf_generator.py::_create_corp_financial_summary`: tabla reestructurada a 6 columnas; `CORP_COLUMNS=['Derecho de Uso','Infraestructura','Apoyo Técnico','Soporte y Monitoreo']`; SPAN de "Hardware y Software" sobre las 2 primeras subcolumnas y "Consultoría" sobre las 2 siguientes. Clasificación de montos por `tipo_corp` automática. Gating ya existente: `generate_vpos_corp` solo para CORP (PYME→`generate_vpos` estándar; GATEWAY/LINK_PAGO→flujos propios).
**QA:** testing_agent iteration_261 → **backend 100% (5/5) + frontend 100% (8/8)**. Verificado visualmente: Set-up $42/$25.50, Recurrentes $10/$22.58, totales $67.50/$32.58/$100.08; PYME sin columna Infraestructura. Test de regresión: `tests/test_iter261_infraestructura_tipo_corp.py`. ⚠️ PREVIEW; requiere REDEPLOY.
...
**Fix:** (1) el payload de PG/LP ahora envía `client_segment: quoteData.client_segment` (el segmento del menú CORP/PYME es autoritativo); (2) `QuoteCreateWithPDF.client_segment` default → `Optional[str] = None`, restaurando el fallback a `user_sede` para cualquier consumidor que omita el campo.
**QA:** testing_agent iteration_241 → **backend 100% (8/8)** + frontend spot-check OK (badge 'Corporativo' al abrir wizard vía CORP; mmartin ve su cotización PG-CORP en el listado). Test: `/app/backend/tests/test_iter241_pg_client_segment_corp.py`. ⚠️ PREVIEW; requiere REDEPLOY. Las cotizaciones YA guardadas como PYME en producción no se auto-corrigen (posible script de reparación).
...
- **Backend (`routes/clients.py`):** helpers `_derive_hierarchy`, `_consolidated_contacts_for_client`, `INHERITED_BRANCH_FIELDS` (Estatus/Definición Legal, Capacidad Operativa, Gestión y Soluciones, Dirección, Info Adicional). Endpoints: `GET /clients/by-rif?rif=` (árbol {principal,branches,has_branches}), `GET /clients/{id}/branches`, `GET /clients/{id}/consolidated-contacts` (Principal globales scope=principal + locales scope=local), `POST /clients/{parent_id}/branches` (acepta payload PARCIAL, hereda snapshot incl. legal_name/fantasy_name, NO copia contactos del Principal), `POST /admin/clients/migrate-hierarchy` (idempotente). `create_client`/`update_client` derivan la jerarquía automáticamente (PUT sin los campos NO rompe el vínculo). `delete_client` bloquea (400) borrar un Principal con sucursales. Migración auto en startup background guardada por flag `config.client_hierarchy_migrated` (Producción se migra al desplegar).
- **Contactos:** viven en cada documento → los del Principal son globales; los de la hija, locales/exclusivos. La consolidación (para envíos) combina Principal + hija.
- **Proyectos (`routes/projects.py::get_suggested_contacts`):** consolida contactos Principal + sucursal con `scope`; email principal/legacy solo del Principal (sin duplicar).
- **Cotizaciones:** `QuoteWizardDialog.jsx` muestra selector obligatorio "Sucursal destino" (`quote-sucursal-block`/`quote-sucursal-select`) cuando el RIF del cliente tiene sucursales (computado desde la lista `clients` por RIF); cambia `client_id`. `Quotes.jsx::handleSendToClient` usa `/clients/{id}/consolidated-contacts`; `QuoteModals.jsx` muestra badges `contact-scope-principal/local-{i}`.
- **Clientes (`Clients.jsx`):** grid con `branch-badge-<id>` (hijas) y `branch-count-<id>` (chip +N en Principales); acción `add-branch-<id>` (solo Principales) abre el form con RIF bloqueado (heredado), datos precargados y panel `inherited-contacts-panel` (contactos globales read-only) + sección de contactos locales; guarda vía `/branches` (botón "Crear Sucursal").
**QA:** testing_agent iteration_237 → **backend 100% (10/10 pytest)** + **frontend 100%** (grid badges, Add Branch dialog con herencia y panel de contactos globales, selector de sucursal en cotización). Gap menor (422 en payload mínimo) **corregido**: el endpoint ahora acepta payload parcial y hereda legal_name/fantasy_name. Migración: 516 principales, 18 sucursales. Test: `tests/test_iter237_client_hierarchy.py`. ⚠️ PREVIEW; requiere REDEPLOY. Ejemplo: RIF J-40064040-7 (Principal cli_0744288d46bf 'Lacoste', sucursal cli_e807e33ae011).
...
**Notificaciones continúan en fases de Implementación · 2026-06-03:** `update_bank_integration` (banks.py) ahora despacha el motor dinámico con el MISMO action_id `new_product_phase_change` (misma config de correos/plantillas/variables) en los cambios de fase de Implementación: PreProd → Primer Prod → Masificación. Se reemplazó el email simulado (log). Se agregó `phase_changed_at` por integración para calcular `dias_fase_saliente`; `dias_totales_proyecto` se mide desde la creación del producto original (source_product_id); `responsable_fase_entrante` se toma de update_data/integración. Validado E2E: PreProd→Primer Prod renderiza todas las variables (saliente=5d, totales, responsable, tipo_evento). El catálogo describe ahora el ciclo completo.

**Asignación SQA + nombres compuestos + fix config borrada · 2026-06-03:**
...
**Bugfix Procesador del Patrocinador de Pinpads (Incidencia A) + Modelo de Pinpad (Incidencia B) · 2026-06-07:**
- Causa raíz A: el frontend (`DirectProjectCreation.jsx`) enviaba `pinpad_processor_id`/`pinpad_processor_name` pero el modelo `DirectProjectCreate` los descartaba → el procesador del pinpad se perdía.
- Fix: agregados `pinpad_processor_id`/`pinpad_processor_name` a `DirectProjectCreate` y mapeados en la synthetic_quote como `sponsor_processor_id`/`sponsor_processor_name`. `quote_transitions._create_project_from_quote` ahora persiste estos campos en el doc del proyecto.
...
**Automatización "Tipo de Proyecto" + Homologación Patrocinio de Pinpads (relacional) · 2026-06-07:**
- PARTE 2 (Proyectos): El campo "Tipo de Proyecto" en "Datos del Proyecto" (ProjectDetail.jsx ~976) ahora hereda AUTOMÁTICAMENTE el `quote_type` de la cotización origen, mostrando 4 valores canónicos: VPOS / MPOS / Payment Gateway / Link de Pago (antes mostraba el valor mergeado "VPOS / MPOS" desde project_type_impl). Mapeo: VPOS→VPOS, MPOS/FAST_TRACK→MPOS, GATEWAY→Payment Gateway, LINK_PAGO/LINK→Link de Pago; fallback legacy a project_type_impl.
- PARTE 3 (Cotizaciones): El campo "Entidad Patrocinadora" (patrocinio de Pinpads, sponsor_bank_id) en QuoteWizardDialog.jsx ahora aplica la MISMA lógica relacional Procesador→Banco que "Banco Patrocinante". handlePinpadSponsorBankSelect: si type==='Procesador' abre sub-modal `pinpad-processor-link-modal` (obligatorio, lista solo bancos vinculados b.procesador===proc.name) y bloquea avance; banco tradicional guarda directo. Etiqueta compuesta `pinpad-sponsor-compound-label` "Patrocinador: [Procesador] — [Banco]".
- Backend: nuevos campos `sponsor_processor_id`/`sponsor_processor_name` en models.py (QuoteCreate, Quote, Project, PDF model) y routes/quotes.py (QuoteCreateWithPDF, QuoteUpdate, ambas construcciones de Quote, PDF de propuesta con "Entidad Patrocinadora" compuesta). Se propagan al proyecto vía quote_transitions (ya existente). 
- Validado: Parte 2 screenshot (project-type-impl='VPOS'); Parte 3 backend curl (create-with-pdf persiste sponsor_processor_name); Parte 3 frontend testing_agent 100% (Procesador→sub-modal 11 bancos vinculados+label compuesta; banco tradicional sin modal; cancelar funciona). Iter29.
- Deuda técnica detectada (no abordada, fuera de alcance): QuoteWizardDialog.jsx >2300 líneas; lógica Procesador→Banco duplicada (candidata a hook compartido useProcessorLinkedBankSelector); 'X is not defined' preexistente en QuoteWizardDialog.jsx ~línea 1067 (latente, no en ruta de uso normal).

**Mejora: Filtro "Tipo de Proyecto" en listado de Proyectos · 2026-06-07:**
- El listado de Proyectos (Projects.jsx) ya mostraba el chip `ProjectTypeBadge` en la columna "Tipo". Se agregó un FILTRO por tipo (dropdown `project-type-filter` con opciones VPOS / MPOS / Payment Gateway / Link de Pago) y los chips de cada fila ahora son CLICKEABLES (`project-type-chip-{id}`) para filtrar por ese tipo (toggle). Estado `typeFilter` + helper `normalizeProjectType` (LINK_PAGO/LINK→LINK, FAST_TRACK→MPOS).
- Bugfix en ProjectTypeBadge.jsx: ahora normaliza `LINK_PAGO` y `FAST_TRACK` (antes mostraban "—" en proyectos directos); label de GATEWAY corregido de "Payment" a "Payment Gateway".
- Validado: screenshot — 71 proyectos → 55 al filtrar VPOS, todos los badges visibles = VPOS, dropdown resaltado. Self-test (cambio pequeño). Lint: 2 errores PREEXISTENTES en Projects.jsx (línea 137 set-state-in-effect, línea 834 'fetchStats' no definido) no relacionados con este cambio.

**Mejora: Reporte de Carga (PDF) agrupable por Tipo de Proyecto · 2026-06-07:**
- El endpoint `GET /api/projects/reports/workload-pdf` ahora acepta `group_by` ('implementer' default | 'type'). En modo 'type' el PDF se segmenta por Tipo de Proyecto (VPOS / MPOS / Payment Gateway / Link de Pago / Sin Tipo) con: encabezado morado por sección + subtotales (Nro Proyectos, Cajas, PVV), columna "Implementador" en cada fila, y un bloque final "RESUMEN DEL MIX POR TIPO DE PROYECTO" con tarjetas de % del total/cajas/PVV. Modo 'implementer' inalterado (ranking por PVV).
- Backend (routes/projects.py): helper `_norm_type` (LINK_PAGO→LINK, FAST_TRACK→MPOS) aplicado también al filtro quote_type; `_TYPE_FULL_LABEL` para encabezados; rama type-grouping autocontenida (inline styles, sin tocar CSS existente); subtítulo dinámico "Agrupado por {group_label}".
- Frontend (WorkloadReportFiltersModal.jsx): toggle "Agrupar el reporte por" (Implementador / Tipo de Proyecto, testids groupby-implementer/groupby-type) con texto explicativo dinámico; envía `group_by` en la query.
- Validado: curl — type/implementer/type+VPOS responden 200; extracción PDF confirma secciones por tipo con subtotales (VPOS 55/MPOS 14/Payment Gateway 1/Link de Pago 1), columna Implementador, resumen mix (77%/20%/1%/1%), filtro VPOS=100%; regresión implementador OK. Screenshot del toggle. Lint backend limpio; 6 errores eslint PREEXISTENTES en el modal (líneas 43/93/116) no relacionados.

**Refactor & Limpieza (Deuda Técnica P3) · 2026-06-07:**
- **Refactor (g):** Extraído componente compartido `/app/frontend/src/components/shared/ProcessorLinkBankModal.jsx` (~80 líneas, DRY) que reemplaza la UI duplicada del sub-modal relacional "Procesador → Banco" en 4 lugares: QuoteWizardDialog (processorModal + pinpadProcessorModal) y DirectProjectCreation (processorModal + pinpadProcessorModal). data-testids y comportamiento preservados EXACTAMENTE. Validado por testing_agent (iteration_30.json): 7/7 escenarios PASS, CERO regresiones.
- **a11y (i):** Agregado `DialogDescription` al componente compartido, al serials-bank-modal (QuoteModals) y al diálogo principal del wizard (QuoteWizardDialog) → resueltos los warnings Radix "Missing Description" de esos modales.
- **Limpieza de lint (h):** Corregidos TODOS los errores reales: react/no-unescaped-entities (QuoteModals 5, QuoteWizardDialog 2, WorkloadModal 1 → &quot;); `X` icon import faltante (QuoteWizardDialog); `fetchStats` indefinido (Projects); componente anidado `Chip` → movido a nivel módulo (WorkloadModal); efecto de carga refactorizado a patrón async con guard de cancelación (WorkloadModal); consolidado `fetchImplementers` dentro de `fetchProjects` (Projects, elimina función redundante).
- **Hydration warning:** Investigado — proviene del wrapper de plataforma Emergent `data-ve-dynamic` (`<span>` alrededor de filas de tabla en /quotes), NO del código de la app (QuotesTable). No corregible a nivel de código.
- **Lint pendiente (no corregible / no bloqueante para el build real):** 1 falso positivo `react-hooks/set-state-in-effect` en `openAssignDialog` (Projects.jsx) — handler onClick estándar. Es una regla experimental del React Compiler (eslint-plugin-react-hooks v6) que NO está en la versión del proyecto (5.2.0); no puede suprimirse con eslint-disable (rompería el build CRA que desconoce la regla) y hace whack-a-mole sobre handlers normales. **El build CRA está VERDE y la app funciona.**
- Documentación técnica integral (P3-f): DIFERIDA por el usuario hasta estabilizar la app.

**Bugfix RBAC: "No tiene acceso al módulo 'dispositivos'" al abrir Proyectos Directos · 2026-06-07:**
- Causa raíz: el middleware RBAC (server.py) mapea `/api/hardware` → módulo `dispositivos`. Un usuario con "Edición Total en Proyectos Directos" pero SIN el módulo `dispositivos` recibía 403 al cargar la página (Promise.all de DirectProjectCreation: /clients, /banks, /integrators, /hardware), abortando toda la carga.
...
**Feature: Módulo "Condiciones Banco/Medio de Pago" (biblioteca de tips implementadores) · 2026-07-14:**
- Nuevo módulo RBAC `condiciones_banco_mediopago` (permissions_catalog.py + usePermission.js) en grupo Gestión de Implementación. Niveles: read (consulta) / edit (guardar).
- Backend routes/bank_payment_conditions.py (registrado en server.py): GET /banco-mediopago-condiciones/banks, GET .../medios-pago (filtro estricto service_type='Producto' AND application_type='setup' → 30 registros), GET .../condicion?bank_id=&service_id=, POST .../condicion (upsert, requiere edit, límite 2000 chars → 400). Colección `banco_mediopago_condiciones` (única por par banco+medio).
- Frontend pages/BankPaymentConditions.jsx + ruta /condiciones-banco-mediopago + ítem en Sidebar (Gestión de Implementación). Selects Banco/Medio, Textarea maxLength 2000 con contador; botón Guardar solo con canEdit; modo read-only deshabilita textarea y oculta guardar (banner de aviso).
- Verificado: backend curl (35 bancos, 30 medios, save/read/persistencia OK, límite 2001→400, read-only→403 al guardar/200 al leer). Frontend admin (save visible, 35/30 opciones) y read-only real Jrojas (banner ON, save oculto, textarea disabled, selects usables).

**Bug fix: firma "CRM - Gestor" en correos "Otras Notificaciones" (ad-hoc de Proyecto) · 2026-07-14:**
- Causa raíz: los endpoints ad-hoc usaban `resolve_project_template_vars` cuyo baseline fija `Firma_Notificacion_Global = build_signature_html(None)` → "CRM - Gestor", sin sobreescribir con el usuario en sesión.
