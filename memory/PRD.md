...
- **QA:** self-test backend — render `_render('{Detalles_Recepcion_Equipos}')` substituye OK; variable presente en `/api/other-actions/metadata`; POST `/api/taller/recepcion` con `detalles_recepcion` → 200 (con cleanup). Screenshot UI: textarea en la posición correcta con contador 71/500. ⚠️ PREVIEW; requiere REDEPLOY. Nota: para que Operaciones vea el texto, el admin debe insertar `{Detalles_Recepcion_Equipos}` en la plantilla del evento en Configuración → Otras Acciones.
