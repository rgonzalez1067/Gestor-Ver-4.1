...
import { Send, Eye, Edit3, Plus, Paperclip, FileText, X, Upload } from 'lucide-react';
...
export const InitialContactEmailDialog = ({ open, onClose, contact, onSent }) => {
  ...
  useEffect(() => {
    if (open && contact) {
      ...
      setPreviewData(null);
      // Plantillas específicas para contactos iniciales; si no hay, caer a CLIENTES.
      api.get('/email-templates?context=INITIAL_CONTACTS').then(r => {
        const rows = r.data || [];
        if (rows.length > 0) {
          setTemplates(rows);
        } else {
          api.get('/email-templates?context=CLIENTES').then(r2 => setTemplates(r2.data || [])).catch(() => setTemplates([]));
        }
      }).catch(() => setTemplates([]));
      // Biblioteca de documentos de comunicación reutilizada de clientes
  ...
  const resolveContactVars = (text) => {
    if (!text || !contact) return text;
    const vars = {
      nombre: contact.contact_name || '',
      contacto: contact.contact_name || '',
      razon_social: contact.legal_name || '',
      empresa: contact.legal_name || '',
      email: contact.email || '',
      telefono: contact.phone || '',
      referido_por: contact.referred_by || '',
      asignado_a: contact.assigned_to_name || '',
      aspectos_interes: contact.interest_notes || '',
    };
    let result = text;
  ...
  const handleTemplateSelect = (tpl) => {
    setSubject(resolveContactVars(tpl.subject || ''));
    setMessage(resolveContactVars(tpl.body_html || tpl.body || ''));
    setPreviewMode(false);
  };

  const insertVariable = (varKey) => setMessage(prev => prev + varKey);
  const handleFileSelect = (e) => { setExternalFiles(prev => [...prev, ...Array.from(e.target.files || [])]); e.target.value = ''; };
  const toggleInternalDoc = (docId) => setSelectedInternalDocs(prev => prev.includes(docId) ? prev.filter(id => id !== docId) : [...prev, docId]);

  const handlePreview = async () => {
    if (!contact) return;
    try {
      const res = await api.post(`/initial-contacts/${contact.contact_id}/preview-email`, { subject, message });
  ...
  const handleSend = async () => {
    ...
    if (!valid.length) { toast.error('Agregue al menos un destinatario válido'); return; }
    ...
    try {
      ...
      formData.append('recipients', JSON.stringify(valid));
      ...
      const res = await api.post(`/initial-contacts/${contact.contact_id}/send-email`, formData, {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden p-0" data-testid="initial-contact-email-dialog">
          ...
          <div className="w-72 bg-slate-50 border-r border-slate-200 p-4 overflow-y-auto flex-shrink-0">
            ...
            <div className="space-y-1.5 mb-6">
              {templates.length === 0 ? (
                ...
                <button key={tpl.template_id} onClick={() => handleTemplateSelect(tpl)}
            ...
            <div className="flex flex-wrap gap-1 mb-6">
              {VARIABLES.map(v => (
                <button key={v.key} onClick={() => insertVariable(v.key)}
                  ...
                  title={v.desc} data-testid={`var-${v.key.replace(/[{}]/g, '')}`}>
                  {v.key}
                </button>
              ))}
            </div>
            ...
            <div className="space-y-1.5">
              {internalDocs.length === 0 ? (
                ...
                <button key={doc.document_id} onClick={() => toggleInternalDoc(doc.document_id)}
          ...
          <div className="flex-1 flex flex-col p-5 overflow-y-auto">
            <DialogHeader className="mb-4">
              <DialogTitle className="flex items-center gap-2 text-base">
                ...
                Enviar Notificación a Contacto Inicial
              </DialogTitle>
              <DialogDescription className="text-xs">
                {contact.contact_name} — {contact.legal_name}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-3 flex-1">
              <div>
                ...
                {recipients.map((r, idx) => (
                  <div key={idx} className="flex gap-2 mt-1">
                    <Input value={r} onChange={(e) => updateRecipient(idx, e.target.value)}
                      placeholder="correo@ejemplo.com" className="text-sm" data-testid={`ic-recipient-${idx}`} />
                    {idx > 0 && (
              ...
              <div>
                ...
                <Input value={subject} onChange={(e) => setSubject(e.target.value)}
                  placeholder="Asunto del correo..." className="text-sm font-semibold" data-testid="ic-email-subject" />
              </div>
              ...
              <div className="flex-1">
                ...
                {previewMode ? (
                  ...
                  <Textarea value={message} onChange={(e) => setMessage(e.target.value)}
                    placeholder="Escriba su mensaje. Use variables como {{contacto}} o {{razon_social}}..."
                    rows={8} className="text-sm resize-none" data-testid="ic-email-message" />
                )}
              </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {externalFiles.map((f, idx) => (
                      <span key={idx} className="flex items-center gap-1 bg-slate-100 text-xs px-2 py-1 rounded">
                        ...
                        <button onClick={() => setExternalFiles(prev => prev.filter((_, i) => i !== idx))}>
            ...
            <div className="pt-4 border-t flex justify-end gap-2">
              ...
              <Button onClick={handleSend} disabled={sending} className="bg-slate-900 hover:bg-slate-800" data-testid="ic-send-email-btn">
...
export default InitialContactEmailDialog;
