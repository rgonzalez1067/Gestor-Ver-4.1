...
import { Plus, Pencil, Trash2, Upload, FileSpreadsheet, FileText, Download, Search, Filter, Users, CheckCircle, Clock, XCircle, AlertTriangle, Info, X } from 'lucide-react';
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto">
          ...
          {showImportResult && importResult && (
            <div className={`rounded-lg border-2 p-4 mb-6 ${getImportStatusColor(importResult.status)}`} data-testid="import-result-panel">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  {getImportStatusIcon(importResult.status)}
                  <div>
                    <h3 className="font-semibold text-slate-900 text-lg">
                      {importResult.status === 'success' && 'Importación Exitosa'}
                      {importResult.status === 'partial' && 'Importación Parcial'}
                      {importResult.status === 'error' && 'Error en Importación'}
                    </h3>
                    <p className="text-slate-600 mt-1">{importResult.message}</p>
                  </div>
                </div>
                <Button variant="ghost" size="sm" onClick={closeImportResult} className="text-slate-500 hover:text-slate-700">
                  <X size={18} />
                </Button>
              </div>
              ...
              <div className="flex gap-6 mt-4 pt-4 border-t border-slate-200">
                <div className="text-center">
                  <div className="text-2xl font-bold text-slate-900">{importResult.total_processed}</div>
                          ...
                          <tr key={idx} className="hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2">
                              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                                err.error_type === 'missing' ? 'bg-red-100 text-red-700' :
                                err.error_type === 'invalid' ? 'bg-amber-100 text-amber-700' :
                                err.error_type === 'duplicate' ? 'bg-blue-100 text-blue-700' :
                  ...
                  {filteredIntegrators.length === 0 ? (
                    ...
                    filteredIntegrators.map((integrator) => (
                      <tr key={integrator.integrator_id} className="hover:bg-slate-50 transition-colors" data-testid={`integrator-row-${integrator.integrator_id}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          {getStatusBadge(integrator.integrator_status)}
                        </td>
                        <td className="px-4 py-3">
